...
class ExcelBridgeDirect(DirectCOMBridgeBase):
    """Excel桥接器 - 直接COM调用"""

    def __init__(self):
        super().__init__(
            script_dir=current_dir,
            bridge_name="ExcelBridgeDirect",
            client_attr="excel",
            app_client=ExcelCOM(),
        )
    ...
    def new(self) -> Dict[str, Any]:
        """新建Excel文件"""
        return super().new(fallback_name="新建文件.xlsx")
    ...
    def format_range(
        ...
        """格式化范围"""
        return self.excel.format_range(range_str, bold, font_size, font_color, number_format, sheet_name)
