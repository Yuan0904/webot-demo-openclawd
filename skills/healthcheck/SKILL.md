---
name: healthcheck
description: Host security hardening and risk-tolerance configuration for WebBot deployments. Use when a user asks for security audits, firewall/SSH/update hardening, risk posture, exposure review, WebBot cron scheduling for periodic checks, or version status checks on a machine running WebBot (laptop, workstation, Pi, VPS).
---

# WebBot Host Hardening

## Overview

Assess and harden the host running WebBot, then align it to a user-defined risk tolerance without breaking access. Use WebBot security tooling as a first-class signal, but treat OS hardening as a separate, explicit set of steps.
...
- Never claim WebBot changes the host firewall, SSH, or OS updates; it does not.
...
5. WebBot gateway status and bind address.
...
9. OS automatic security updates status.
   Note: these are not blocking items, but are highly recommended, especially if WebBot can access sensitive data.
...
### 2) Run WebBot security audits (read-only)

As part of the default read-only checks, run `webbot security audit --deep`. Only offer alternatives if the user requests them:

1. `webbot security audit` (faster, non-probing)
2. `webbot security audit --json` (structured output)

Offer to apply WebBot safe defaults (numbered):

1. `webbot security audit --fix`

Be explicit that `--fix` only tightens WebBot defaults and file permissions. It does not change host firewall, SSH, or OS update policies.
...
### 3) Check WebBot version/update status (read-only)

As part of the default read-only checks, run `webbot update status`.
...
- Credential hygiene notes (location of WebBot creds, prefer disk encryption)
...
After WebBot install or first hardening pass, run at least one baseline audit and version check:

- `webbot security audit`
- `webbot security audit --deep`
- `webbot update status`

Ongoing monitoring is recommended. Use the WebBot cron tool/CLI to schedule periodic audits (Gateway scheduler). Do not create scheduled tasks without explicit approval. Store outputs in a user-approved location and avoid secrets in logs.
...
1. “Do you want me to schedule periodic audits (e.g., daily/weekly) via `webbot cron add`?”

If the user says yes, ask for:

- cadence (daily/weekly), preferred time window, and output location
- whether to also schedule `webbot update status`
...
Before creating, `webbot cron list` and match on exact `name`. If found, `webbot cron edit <id> ...`.
If not found, `webbot cron add --name <name> ...`.

Also offer a periodic version check so the user can decide when to update (numbered):

1. `webbot update status` (preferred for source checkouts and channels)
2. `npm view webbot version` (published npm version)

## WebBot command accuracy

Use only supported commands and flags:

- `webbot security audit [--deep] [--fix] [--json]`
- `webbot status` / `webbot status --deep`
- `webbot health --json`
- `webbot update status`
- `webbot cron add|list|runs|run`

Do not invent CLI flags or imply WebBot enforces host firewall/SSH policies.
...
Follow the durable-memory prompt format used by WebBot compaction:
