...
`webbot.invoke --each`) without adding new WebBot code per workflow.
...
This extension depends on WebBot internal modules (the embedded agent runner).
It is intended to ship as a **bundled** WebBot extension (like `lobster`) and
be enabled via `plugins.entries` + tool allowlists.

It is **not** currently designed to be copied into
`~/.webbot/extensions` as a standalone plugin directory.
