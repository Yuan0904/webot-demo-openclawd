...
metadata: { "webbot": { "emoji": "🪶", "homepage": "https://www.prose.md" } }
---
...
## WebBot Runtime Mapping

- **Task tool** in the upstream spec == WebBot `sessions_spawn`
- **File I/O** == WebBot `read`/`write`
- **Remote fetch** == WebBot `web_fetch` (or `exec` with curl when POST is required)
