...
homepage: https://docs.webbot.ai/hooks#command-logger
metadata:
  {
    "webbot":
      {
...
2. **Appends to log file** - Writes a JSON line to `~/.webbot/logs/commands.log`
...
1. **Manual rotation**:
   ...
   ```bash
   mv ~/.webbot/logs/commands.log ~/.webbot/logs/commands.log.old
   ```

2. **Use logrotate** (Linux):
   Create `/etc/logrotate.d/webbot`:
   ```
   /home/username/.webbot/logs/commands.log {
       weekly
       rotate 4
       compress
       missingok
...
```bash
tail -n 20 ~/.webbot/logs/commands.log
```

Pretty-print with jq:

```bash
cat ~/.webbot/logs/commands.log | jq .
```

Filter by action:

```bash
grep '"action":"new"' ~/.webbot/logs/commands.log | jq .
```
