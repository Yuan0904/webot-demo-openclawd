...
This directory contains hooks that ship with WebBot. These hooks are automatically discovered and can be enabled/disabled via CLI or configuration.
...
**Output**: `<workspace>/memory/YYYY-MM-DD-slug.md` (defaults to `~/.webbot/workspace`)

**Enable**:

```bash
webbot hooks enable session-memory
```
...
**Output**: `~/.webbot/logs/commands.log`

**Enable**:

```bash
webbot hooks enable command-logger
```
...
**Docs**: https://docs.webbot.ai/hooks/soul-evil

**Enable**:

```bash
webbot hooks enable soul-evil
```
...
homepage: https://docs.webbot.ai/hooks#my-hook
metadata:
  { "webbot": { "emoji": "🔗", "events": ["command:new"], "requires": { "bins": ["node"] } } }
---
# Hook Title
...
- **Managed hooks**: `~/.webbot/hooks/` (shared across workspaces)
...
```bash
webbot hooks info session-memory
```
...
```bash
webbot hooks enable session-memory
webbot hooks disable command-logger
```

## Configuration

Hooks can be configured in `~/.webbot/webbot.json`:
...
2. Restart gateway: `pkill -9 -f 'webbot.*gateway' && pnpm webbot gateway`
3. Enable the hook: `webbot hooks enable my-hook`
4. Trigger the event (e.g., send `/new` command)
5. Check gateway logs for hook execution

## Documentation

Full documentation: https://docs.webbot.ai/hooks
