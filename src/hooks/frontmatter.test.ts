...
`;
    ...
    expect(parsed.webbot.emoji).toBe("💾");
    expect(parsed.webbot.events).toEqual(["command:new"]);
  });
...
`;
    ...
    const parsed = JSON.parse(result.metadata);
    expect(parsed.webbot.emoji).toBe("📝");
    expect(parsed.webbot.events).toEqual(["command"]);
    expect(parsed.webbot.requires.config).toEqual(["workspace.dir"]);
    expect(parsed.webbot.install[0].kind).toBe("bundled");
  });

  it("handles single-line metadata (inline JSON)", () => {
    const content = `---
name: simple-hook
metadata: {"webbot": {"events": ["test"]}}
---
`;
    const result = parseFrontmatter(content);
    expect(result.name).toBe("simple-hook");
    expect(result.metadata).toBe('{"webbot": {"events": ["test"]}}');
  });
...
describe("resolveWebBotMetadata", () => {
  it("extracts webbot metadata from parsed frontmatter", () => {
    const frontmatter = {
      name: "test-hook",
    ...
    const result = resolveWebBotMetadata(frontmatter);
  ...
  it("returns undefined when webbot key is missing", () => {
    const frontmatter = {
  ...
  it("handles install specs", () => {
    const frontmatter = {
      metadata: JSON.stringify({
        webbot: {
          events: ["command"],
          install: [
            { id: "bundled", kind: "bundled", label: "Bundled with WebBot" },
            { id: "npm", kind: "npm", package: "@webbot/hook" },
          ],
        },
      }),
    };
    ...
    expect(result?.install?.[1].package).toBe("@webbot/hook");
  });
...
homepage: https://docs.webbot.ai/hooks#session-memory
metadata:
  {
    "webbot":
      {
        ...
        "events": ["command:new"],
        "requires": { "config": ["workspace.dir"] },
        "install": [{ "id": "bundled", "kind": "bundled", "label": "Bundled with WebBot" }],
      },
  }
---
...
`;
    ...
    const webbot = resolveWebBotMetadata(frontmatter);
    expect(webbot).toBeDefined();
    expect(webbot?.emoji).toBe("💾");
    expect(webbot?.events).toEqual(["command:new"]);
    expect(webbot?.requires?.config).toEqual(["workspace.dir"]);
    expect(webbot?.install?.[0].kind).toBe("bundled");
  });
