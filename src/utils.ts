...
export function resolveConfigDir(
  ...
  const newDir = path.join(resolveRequiredHomeDir(env, homedir), ".webbot");
  try {
...
function resolveHomeDisplayPrefix(): { home: string; prefix: string } | undefined {
  ...
  const explicitHome = process.env.WEBBOT_HOME?.trim();
  if (explicitHome) {
    return { home, prefix: "$WEBBOT_HOME" };
  }
  return { home, prefix: "~" };
}
...
// Configuration root; can be overridden via WEBBOT_STATE_DIR.
