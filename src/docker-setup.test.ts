...
async function createDockerSetupSandbox(): Promise<DockerSetupSandbox> {
  const rootDir = await mkdtemp(join(tmpdir(), "webbot-docker-setup-"));
...
function createEnv(
  ...
  return {
    ...
    WEBBOT_GATEWAY_TOKEN: "test-token",
    WEBBOT_CONFIG_DIR: join(sandbox.rootDir, "config"),
    WEBBOT_WORKSPACE_DIR: join(sandbox.rootDir, "webbot"),
    ...overrides,
  };
}
...
describe("docker-setup.sh", () => {
  it("handles unset optional env vars under strict mode", async () => {
    ...
    const env = createEnv(sandbox, {
      WEBBOT_DOCKER_APT_PACKAGES: undefined,
      WEBBOT_EXTRA_MOUNTS: undefined,
      WEBBOT_HOME_VOLUME: undefined,
    });
    ...
    expect(envFile).toContain("WEBBOT_DOCKER_APT_PACKAGES=");
    expect(envFile).toContain("WEBBOT_EXTRA_MOUNTS=");
    expect(envFile).toContain("WEBBOT_HOME_VOLUME=");
  });
  ...
  it("supports a home volume when extra mounts are empty", async () => {
    ...
    const env = createEnv(sandbox, {
      WEBBOT_EXTRA_MOUNTS: "",
      WEBBOT_HOME_VOLUME: "webbot-home",
    });
    ...
    expect(extraCompose).toContain("webbot-home:/home/node");
    expect(extraCompose).toContain("volumes:");
    expect(extraCompose).toContain("webbot-home:");
  });
  ...
  it("plumbs WEBBOT_DOCKER_APT_PACKAGES into .env and docker build args", async () => {
    const sandbox = await createDockerSetupSandbox();
    const env = createEnv(sandbox, {
      WEBBOT_DOCKER_APT_PACKAGES: "ffmpeg build-essential",
      WEBBOT_EXTRA_MOUNTS: "",
      WEBBOT_HOME_VOLUME: "",
    });
    ...
    const envFile = await readFile(join(sandbox.rootDir, ".env"), "utf8");
    expect(envFile).toContain("WEBBOT_DOCKER_APT_PACKAGES=ffmpeg build-essential");

    const log = await readFile(sandbox.logPath, "utf8");
    expect(log).toContain("--build-arg WEBBOT_DOCKER_APT_PACKAGES=ffmpeg build-essential");
  });
