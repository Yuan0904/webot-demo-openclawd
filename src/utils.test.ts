...
describe("ensureDir", () => {
  it("creates nested directory", async () => {
    const tmp = await fs.promises.mkdtemp(path.join(os.tmpdir(), "webbot-test-"));
...
describe("jidToE164", () => {
  ...
  it("maps @lid from authDir mapping files", () => {
    const authDir = fs.mkdtempSync(path.join(os.tmpdir(), "webbot-auth-"));
  ...
  it("falls back through lidMappingDirs in order", () => {
    const first = fs.mkdtempSync(path.join(os.tmpdir(), "webbot-lid-a-"));
    const second = fs.mkdtempSync(path.join(os.tmpdir(), "webbot-lid-b-"));
...
describe("resolveConfigDir", () => {
  it("prefers ~/.webbot when legacy dir is missing", async () => {
    const root = await fs.promises.mkdtemp(path.join(os.tmpdir(), "webbot-config-dir-"));
    try {
...
describe("resolveHomeDir", () => {
  it("prefers WEBBOT_HOME over HOME", () => {
    vi.stubEnv("WEBBOT_HOME", "/srv/webbot-home");
    vi.stubEnv("HOME", "/home/other");

    expect(resolveHomeDir()).toBe(path.resolve("/srv/webbot-home"));
...
describe("shortenHomePath", () => {
  it("uses $WEBBOT_HOME prefix when WEBBOT_HOME is set", () => {
    vi.stubEnv("WEBBOT_HOME", "/srv/webbot-home");
    vi.stubEnv("HOME", "/home/other");

    expect(shortenHomePath(`${path.resolve("/srv/webbot-home")}/.webbot/webbot.json`)).toBe(
      "$WEBBOT_HOME/.webbot/webbot.json",
    );
...
describe("shortenHomeInString", () => {
  it("uses $WEBBOT_HOME replacement when WEBBOT_HOME is set", () => {
    vi.stubEnv("WEBBOT_HOME", "/srv/webbot-home");
    vi.stubEnv("HOME", "/home/other");

    expect(
      shortenHomeInString(`config: ${path.resolve("/srv/webbot-home")}/.webbot/webbot.json`),
    ).toBe("config: $WEBBOT_HOME/.webbot/webbot.json");
...
describe("resolveUserPath", () => {
  ...
  it("expands ~/ to home dir", () => {
    expect(resolveUserPath("~/webbot")).toBe(path.resolve(os.homedir(), "webbot"));
  });
  ...
  it("prefers WEBBOT_HOME for tilde expansion", () => {
    vi.stubEnv("WEBBOT_HOME", "/srv/webbot-home");
    vi.stubEnv("HOME", "/home/other");

    expect(resolveUserPath("~/webbot")).toBe(path.resolve("/srv/webbot-home", "webbot"));
