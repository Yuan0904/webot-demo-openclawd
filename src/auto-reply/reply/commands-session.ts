...
import { scheduleGatewaySigusr1Restart, triggerWebBotRestart } from "../../infra/restart.js";
...
export const handleRestartCommand: CommandHandler = async (params, allowTextCommands) => {
  ...
  if (hasSigusr1Listener) {
    ...
    return {
      shouldContinue: false,
      reply: {
        text: "⚙️ Restarting WebBot in-process (SIGUSR1); back in a few seconds.",
      },
    };
  }
  const restartMethod = triggerWebBotRestart();
  if (!restartMethod.ok) {
  ...
  return {
    shouldContinue: false,
    reply: {
      text: `⚙️ Restarting WebBot via ${restartMethod.method}; give me a few seconds to come back online.`,
    },
  };
};
