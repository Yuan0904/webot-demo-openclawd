...
import type { WebBotConfig } from "../config/types.js";
import type {
...
export function isCommandEnabled(cfg: WebBotConfig, commandKey: string): boolean {
...
function resolveDefaultCommandContext(cfg?: WebBotConfig): {
  provider: string;
  model: string;
} {
  const resolved = resolveConfiguredModelRef({
    cfg: cfg ?? ({} as WebBotConfig),
...
export function getCommandDetection(_cfg?: WebBotConfig): CommandDetection {
  const commands = getChatCommands();
...
export function maybeResolveTextAlias(raw: string, cfg?: WebBotConfig) {
