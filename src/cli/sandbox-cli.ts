...
const SANDBOX_EXAMPLES = {
  main: [
    ["webbot sandbox list", "List all sandbox containers."],
    ["webbot sandbox list --browser", "List only browser containers."],
    ["webbot sandbox recreate --all", "Recreate all containers."],
    ["webbot sandbox recreate --session main", "Recreate a specific session."],
    ["webbot sandbox recreate --agent mybot", "Recreate agent containers."],
    ["webbot sandbox explain", "Explain effective sandbox config."],
  ],
  list: [
    ["webbot sandbox list", "List all sandbox containers."],
    ["webbot sandbox list --browser", "List only browser containers."],
    ["webbot sandbox list --json", "JSON output."],
  ],
  recreate: [
    ["webbot sandbox recreate --all", "Recreate all containers."],
    ["webbot sandbox recreate --session main", "Recreate a specific session."],
    ["webbot sandbox recreate --agent mybot", "Recreate a specific agent (includes sub-agents)."],
    ["webbot sandbox recreate --browser --all", "Recreate only browser containers."],
    ["webbot sandbox recreate --all --force", "Skip confirmation."],
  ],
  explain: [
    ["webbot sandbox explain", "Show effective sandbox config."],
    ["webbot sandbox explain --session agent:main:main", "Explain a specific session."],
    ["webbot sandbox explain --agent work", "Explain an agent sandbox."],
    ["webbot sandbox explain --json", "JSON output."],
  ],
} as const;
...
export function registerSandboxCli(program: Command) {
  const sandbox = program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/sandbox", "docs.webbot.ai/cli/sandbox")}\n`,
    )
    .action(() => {
