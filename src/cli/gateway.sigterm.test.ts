...
describe("gateway SIGTERM", () => {
  ...
  it("exits 0 on SIGTERM", { timeout: 180_000 }, async () => {
    const stateDir = fs.mkdtempSync(path.join(os.tmpdir(), "webbot-gateway-test-"));
    const out: string[] = [];
    const err: string[] = [];
    ...
    const env = {
      ...
      WEBBOT_SKIP_CHANNELS: "1",
      WEBBOT_SKIP_GMAIL_WATCHER: "1",
      WEBBOT_SKIP_CRON: "1",
      WEBBOT_SKIP_BROWSER_CONTROL_SERVER: "1",
      WEBBOT_SKIP_CANVAS_HOST: "1",
    };
    const bootstrapPath = path.join(stateDir, "webbot-entry-bootstrap.mjs");
