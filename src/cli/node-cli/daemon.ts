...
function renderNodeServiceStartHints(): string[] {
  const base = [formatCliCommand("webbot node install"), formatCliCommand("webbot node start")];
...
export async function runNodeDaemonInstall(opts: NodeDaemonInstallOptions) {
  ...
  if (loaded && !opts.force) {
    ...
    if (!json) {
      ...
      defaultRuntime.log(`Reinstall with: ${formatCliCommand("webbot node install --force")}`);
    }
    return;
  }
...
export async function runNodeDaemonStatus(opts: NodeDaemonStatusOptions = {}) {
  ...
  const hintEnv = {
    ...baseEnv,
    WEBBOT_LOG_PREFIX: baseEnv.WEBBOT_LOG_PREFIX ?? "node",
