...
async function runGatewayCommand(opts: GatewayRunOpts) {
  const isDevProfile = process.env.WEBBOT_PROFILE?.trim().toLowerCase() === "dev";
  ...
  if (opts.claudeCliLogs) {
    ...
    process.env.WEBBOT_CLAUDE_CLI_LOG_OUTPUT = "1";
  }
  ...
  if (opts.rawStream) {
    process.env.WEBBOT_RAW_STREAM = "1";
  }
  const rawStreamPath = toOptionString(opts.rawStreamPath);
  if (rawStreamPath) {
    process.env.WEBBOT_RAW_STREAM_PATH = rawStreamPath;
  }
  ...
  if (!opts.allowUnconfigured && mode === "remote") {
    defaultRuntime.error(
      `Gateway start blocked: gateway.mode is "remote". Set gateway.mode=local or pass --allow-unconfigured.`,
    );
    defaultRuntime.exit(1);
    return;
  }
  ...
  if (resolvedAuthMode === "token" && !hasToken && !resolvedAuth.allowTailscale) {
    defaultRuntime.error(
      [
        ...
        "Set gateway.auth.token (or WEBBOT_GATEWAY_TOKEN), or pass --token.",
        ...authHints,
      ]
        .filter(Boolean)
        .join("\n"),
  ...
  if (resolvedAuthMode === "password" && !hasPassword) {
    defaultRuntime.error(
      [
        ...
        "Set gateway.auth.password (or WEBBOT_GATEWAY_PASSWORD), or pass --password.",
        ...authHints,
      ]
        .filter(Boolean)
        .join("\n"),
  ...
  if (bind !== "loopback" && !hasSharedSecret) {
    defaultRuntime.error(
      [
        ...
        "Set gateway.auth.token/password (or WEBBOT_GATEWAY_TOKEN/WEBBOT_GATEWAY_PASSWORD) or pass --token/--password.",
        ...authHints,
      ]
        .filter(Boolean)
        .join("\n"),
  ...
  try {
    ...
    if (
      ...
      defaultRuntime.error(
        `Gateway failed to start: ${errMessage}\nIf the gateway is supervised, stop it with: ${formatCliCommand("webbot gateway stop")}`,
      );
      try {
...
export function addGatewayRunCommand(cmd: Command): Command {
  return cmd
    ...
    .option(
      "--token <token>",
      "Shared token required in connect.params.auth.token (default: WEBBOT_GATEWAY_TOKEN env if set)",
    )
