...
describe("daemon-cli coverage", () => {
  const originalEnv = {
    WEBBOT_STATE_DIR: process.env.WEBBOT_STATE_DIR,
    WEBBOT_CONFIG_PATH: process.env.WEBBOT_CONFIG_PATH,
    WEBBOT_GATEWAY_PORT: process.env.WEBBOT_GATEWAY_PORT,
    WEBBOT_PROFILE: process.env.WEBBOT_PROFILE,
  };

  beforeEach(() => {
    process.env.WEBBOT_STATE_DIR = "/tmp/webbot-cli-state";
    process.env.WEBBOT_CONFIG_PATH = "/tmp/webbot-cli-state/webbot.json";
    delete process.env.WEBBOT_GATEWAY_PORT;
    delete process.env.WEBBOT_PROFILE;
    serviceReadCommand.mockResolvedValue(null);
  });

  afterEach(() => {
    if (originalEnv.WEBBOT_STATE_DIR !== undefined) {
      process.env.WEBBOT_STATE_DIR = originalEnv.WEBBOT_STATE_DIR;
    } else {
      delete process.env.WEBBOT_STATE_DIR;
    }

    if (originalEnv.WEBBOT_CONFIG_PATH !== undefined) {
      process.env.WEBBOT_CONFIG_PATH = originalEnv.WEBBOT_CONFIG_PATH;
    } else {
      delete process.env.WEBBOT_CONFIG_PATH;
    }

    if (originalEnv.WEBBOT_GATEWAY_PORT !== undefined) {
      process.env.WEBBOT_GATEWAY_PORT = originalEnv.WEBBOT_GATEWAY_PORT;
    } else {
      delete process.env.WEBBOT_GATEWAY_PORT;
    }

    if (originalEnv.WEBBOT_PROFILE !== undefined) {
      process.env.WEBBOT_PROFILE = originalEnv.WEBBOT_PROFILE;
    } else {
      delete process.env.WEBBOT_PROFILE;
    }
  });
  ...
  it("derives probe URL from service args + env (json)", async () => {
    ...
    serviceReadCommand.mockResolvedValueOnce({
      ...
      environment: {
        WEBBOT_PROFILE: "dev",
        WEBBOT_STATE_DIR: "/tmp/webbot-daemon-state",
        WEBBOT_CONFIG_PATH: "/tmp/webbot-daemon-state/webbot.json",
        WEBBOT_GATEWAY_PORT: "19001",
      },
