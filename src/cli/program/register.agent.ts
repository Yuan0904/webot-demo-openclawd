...
${formatHelpExamples([
  ['webbot agent --to +15555550123 --message "status update"', "Start a new session."],
  ['webbot agent --agent ops --message "Summarize logs"', "Use a specific agent."],
  [
    'webbot agent --session-id 1234 --message "Summarize inbox" --thinking medium',
    "Target a session with explicit thinking level.",
  ],
  [
    'webbot agent --to +15555550123 --message "Trace logs" --verbose on --json',
    "Enable verbose logging and JSON output.",
  ],
  ['webbot agent --to +15555550123 --message "Summon reply" --deliver', "Deliver reply."],
  [
    'webbot agent --agent ops --message "Generate report" --deliver --reply-channel slack --reply-to "#reports"',
    "Send reply to a different channel/target.",
  ],
])}

${theme.muted("Docs:")} ${formatDocsLink("/cli/agent", "docs.webbot.ai/cli/agent")}`,
    )
    .action(async (opts) => {
  ...
  const agents = program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/agents", "docs.webbot.ai/cli/agents")}\n`,
    );
...
${theme.heading("Examples:")}
${formatHelpExamples([
  ['webbot agents set-identity --agent main --name "WebBot" --emoji "🦞"', "Set name + emoji."],
  ["webbot agents set-identity --agent main --avatar avatars/webbot.png", "Set avatar path."],
  [
    "webbot agents set-identity --workspace ~/.webbot/workspace --from-identity",
    "Load from IDENTITY.md.",
  ],
  [
    "webbot agents set-identity --identity-file ~/.webbot/workspace/IDENTITY.md --agent main",
