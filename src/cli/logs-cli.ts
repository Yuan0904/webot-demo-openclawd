...
function createLogWriters() {
  const writer = createSafeStreamWriter({
    ...
    onBrokenPipe: (err: { code?: string }, stream: NodeJS.WritableStream) => {
      const code = err.code ?? "EPIPE";
      const target = stream === process.stdout ? "stdout" : "stderr";
      const message = `webbot logs: output ${target} closed (${code}). Stopping tail.`;
      try {
...
function emitGatewayError(
  ...
  const hint = `Hint: run \`${formatCliCommand("webbot doctor")}\`.`;
...
export function registerLogsCli(program: Command) {
  const logs = program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/logs", "docs.webbot.ai/cli/logs")}\n`,
    );
