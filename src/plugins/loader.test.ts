...
import { loadWebBotPlugins } from "./loader.js";

type TempPlugin = { dir: string; file: string; id: string };

const tempDirs: string[] = [];
const prevBundledDir = process.env.WEBBOT_BUNDLED_PLUGINS_DIR;
const EMPTY_PLUGIN_SCHEMA = { type: "object", additionalProperties: false, properties: {} };

function makeTempDir() {
  const dir = path.join(os.tmpdir(), `webbot-plugin-${randomUUID()}`);
...
function writePlugin(params: {
  ...
  fs.writeFileSync(
    path.join(dir, "webbot.plugin.json"),
    JSON.stringify(
      {
        id: params.id,
...
afterEach(() => {
  ...
  if (prevBundledDir === undefined) {
    ...
    process.env.WEBBOT_BUNDLED_PLUGINS_DIR = prevBundledDir;
  }
});

describe("loadWebBotPlugins", () => {
  it("disables bundled plugins by default", () => {
    ...
    process.env.WEBBOT_BUNDLED_PLUGINS_DIR = bundledDir;

    const registry = loadWebBotPlugins({
      cache: false,
      config: {
        plugins: {
          allow: ["bundled"],
    ...
    const enabledRegistry = loadWebBotPlugins({
      cache: false,
      config: {
        plugins: {
          allow: ["bundled"],
  ...
  it("preserves package.json metadata for bundled memory plugins", () => {
    ...
    fs.writeFileSync(
      ...
      JSON.stringify({
        name: "@webbot/memory-core",
        version: "1.2.3",
        description: "Memory plugin package",
        webbot: { extensions: ["./index.ts"] },
      }),
      "utf-8",
    );
    writePlugin({
