...
import { discoverWebBotPlugins } from "./discovery.js";
...
function resolvePluginModuleExport(moduleExport: unknown): {
  definition?: WebBotPluginDefinition;
  register?: WebBotPluginDefinition["register"];
} {
  const resolved =
    moduleExport &&
  ...
  if (typeof resolved === "function") {
    return {
      register: resolved as WebBotPluginDefinition["register"],
    };
  }
  if (resolved && typeof resolved === "object") {
    const def = resolved as WebBotPluginDefinition;
...
export function loadWebBotPlugins(options: PluginLoadOptions = {}): PluginRegistry {
  ...
  const discovery = discoverWebBotPlugins({
    workspaceDir: options.workspaceDir,
  ...
  const jiti = createJiti(import.meta.url, {
    ...
    ...(pluginSdkAlias
      ? {
          alias: { "webbot/plugin-sdk": pluginSdkAlias },
        }
      : {}),
  });
  ...
  for (const candidate of discovery.candidates) {
    ...
    let mod: WebBotPluginModule | null = null;
    try {
      mod = jiti(candidate.source) as WebBotPluginModule;
    } catch (err) {
