...
const defaultTools = createWebBotCodingTools();

describe("createWebBotCodingTools", () => {
  it("keeps read tool image metadata intact", async () => {
    const readTool = defaultTools.find((tool) => tool.name === "read");
    expect(readTool).toBeDefined();

    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "webbot-read-"));
    try {
  ...
  it("returns text content without image blocks for text files", async () => {
    ...
    try {
      ...
      const contents = "Hello from webbot read tool.";
  ...
  it("filters tools by sandbox policy", () => {
    const sandbox = {
      ...
      workspaceDir: path.join(os.tmpdir(), "webbot-sandbox"),
      agentWorkspaceDir: path.join(os.tmpdir(), "webbot-workspace"),
    ...
    const tools = createWebBotCodingTools({ sandbox });
