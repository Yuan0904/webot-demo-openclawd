...
import { createWebBotCodingTools } from "./pi-tools.js";
...
describe("workspace path resolution", () => {
  it("reads relative paths against workspaceDir even after cwd changes", async () => {
    await withTempDir("webbot-ws-", async (workspaceDir) => {
      await withTempDir("webbot-cwd-", async (otherDir) => {
        ...
        try {
          const tools = createWebBotCodingTools({ workspaceDir });
    ...
    await withTempDir("webbot-ws-", async (workspaceDir) => {
      await withTempDir("webbot-cwd-", async (otherDir) => {
        ...
        try {
          ...
          expect(updated).toBe("hello webbot");
        } finally {
          cwdSpy.mockRestore();
        }
      });
  ...
  it("defaults exec cwd to workspaceDir when workdir is omitted", async () => {
    await withTempDir("webbot-ws-", async (workspaceDir) => {
      const tools = createWebBotCodingTools({ workspaceDir, exec: { host: "gateway" } });
  ...
  it("lets exec workdir override the workspace default", async () => {
    await withTempDir("webbot-ws-", async (workspaceDir) => {
      await withTempDir("webbot-override-", async (overrideDir) => {
        const tools = createWebBotCodingTools({ workspaceDir, exec: { host: "gateway" } });
        const execTool = tools.find((tool) => tool.name === "exec");
        expect(execTool).toBeDefined();
...
describe("sandboxed workspace paths", () => {
  it("uses sandbox workspace for relative read/write/edit", async () => {
    await withTempDir("webbot-sandbox-", async (sandboxDir) => {
      await withTempDir("webbot-workspace-", async (workspaceDir) => {
        const sandbox = {
          enabled: true,
          ...
          workspaceAccess: "rw",
          containerName: "webbot-sbx-test",
          containerWorkdir: "/workspace",
          docker: {
            image: "webbot-sandbox:bookworm-slim",
            containerPrefix: "webbot-sbx-",
            workdir: "/workspace",
            readOnlyRoot: true,
            tmpfs: [],
            network: "none",
        ...
        const tools = createWebBotCodingTools({ workspaceDir, sandbox });
        const readTool = tools.find((tool) => tool.name === "read");
        const writeTool = tools.find((tool) => tool.name === "write");
        const editTool = tools.find((tool) => tool.name === "edit");
