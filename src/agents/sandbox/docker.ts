...
export function buildSandboxCreateArgs(params: {
  ...
  args.push("--label", "webbot.sandbox=1");
  args.push("--label", `webbot.sessionKey=${params.scopeKey}`);
  args.push("--label", `webbot.createdAtMs=${createdAtMs}`);
  if (params.configHash) {
    args.push("--label", `webbot.configHash=${params.configHash}`);
  }
...
async function readContainerConfigHash(containerName: string): Promise<string | null> {
  ...
  return await readLabel("webbot.configHash");
}

function formatSandboxRecreateHint(params: { scope: SandboxConfig["scope"]; sessionKey: string }) {
  if (params.scope === "session") {
    return formatCliCommand(`webbot sandbox recreate --session ${params.sessionKey}`);
  }
  if (params.scope === "agent") {
    const agentId = resolveSandboxAgentId(params.sessionKey) ?? "main";
    return formatCliCommand(`webbot sandbox recreate --agent ${agentId}`);
  }
  return formatCliCommand("webbot sandbox recreate --all");
}
