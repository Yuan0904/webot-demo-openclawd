...
export async function resolveApiKeyForProvider(params: {
  ...
  throw new Error(
    [
      ...
      `Configure auth for this agent (${formatCliCommand("webbot agents add <id>")}) or copy auth-profiles.json from the main agentDir.`,
    ].join(" "),
  );
}
...
export function resolveEnvApiKey(provider: string): EnvApiKeyResult | null {
  ...
  // Generic fallback: WEBBOT_DEFAULT_API_KEY covers any provider configured
  // via the WEBBOT_DEFAULT_* env-var family.
  return pick("WEBBOT_DEFAULT_API_KEY");
}
