...
function resolveExecConfig(cfg: WebBotConfig | undefined) {
...
export function createWebBotCodingTools(options?: {
  ...
  const base = (codingTools as unknown as AnyAgentTool[]).flatMap((tool) => {
    if (tool.name === readTool.name) {
      ...
      return [createWebBotReadTool(freshReadTool)];
    }
