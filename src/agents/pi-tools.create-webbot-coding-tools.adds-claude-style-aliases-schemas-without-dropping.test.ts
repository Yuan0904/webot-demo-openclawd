...
import { __testing, createWebBotCodingTools } from "./pi-tools.js";
...
describe("createWebBotCodingTools", () => {
  ...
  it("does not expose provider-specific message tools", () => {
    const tools = createWebBotCodingTools({ messageProvider: "discord" });
  ...
  it("applies sandbox path guards to file_path alias", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "webbot-sbx-"));
    const outsidePath = path.join(os.tmpdir(), "webbot-outside.txt");
