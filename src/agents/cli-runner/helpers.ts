...
/**
 * Cleanup suspended WebBot CLI processes that have accumulated.
...
function buildModelAliasLines(cfg?: WebBotConfig) {
...
export async function writeCliImages(
  ...
  const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "webbot-cli-images-"));
  const paths: string[] = [];
