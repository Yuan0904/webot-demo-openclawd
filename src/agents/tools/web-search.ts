...
type WebSearchConfig = NonNullable<WebBotConfig["tools"]>["web"] extends infer Web
...
function resolveSearchConfig(cfg?: WebBotConfig): WebSearchConfig {
...
function missingSearchKeyPayload(provider: (typeof SEARCH_PROVIDERS)[number]) {
  ...
  return {
    ...
    message: `web_search needs a Brave Search API key. Run \`${formatCliCommand("webbot configure --section web")}\` to store it, or set BRAVE_API_KEY in the Gateway environment.`,
    docs: "https://docs.webbot.ai/tools/web",
  };
}
...
async function runPerplexitySearch(params: {
  ...
  const res = await fetch(endpoint, {
    ...
    headers: {
      ...
      "HTTP-Referer": "https://webbot.ai",
      "X-Title": "WebBot Web Search",
    },
    body: JSON.stringify({
      model,
      messages: [
