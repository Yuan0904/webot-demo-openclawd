...
describe("media store", () => {
  ...
  const snapshotEnv = () => {
    for (const key of ["HOME", "USERPROFILE", "HOMEDRIVE", "HOMEPATH", "WEBBOT_STATE_DIR"]) {
  ...
  beforeAll(async () => {
    snapshotEnv();
    home = await fs.mkdtemp(path.join(os.tmpdir(), "webbot-test-home-"));
    process.env.HOME = home;
    process.env.USERPROFILE = home;
    process.env.WEBBOT_STATE_DIR = path.join(home, ".webbot");
    ...
    await fs.mkdir(path.join(home, ".webbot"), { recursive: true });
  ...
  it("creates and returns media directory", async () => {
    await withTempStore(async (store, home) => {
      ...
      expect(path.normalize(dir)).toContain(`${path.sep}.webbot${path.sep}media`);
