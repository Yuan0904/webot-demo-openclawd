...
export function formatForLog(value: unknown, depth = 0): string {
  try {
    if (value instanceof Error) {
      ...
      // HTTP status (openai/anthropic APIStatusError)
      const status =
        "status" in value && (typeof value.status === "string" || typeof value.status === "number")
          ? String(value.status)
          : "";
      if (status) {
        parts.push(`status=${status}`);
      }
      // API error body (openai APIStatusError has .error with parsed body)
      const errBody = (value as { error?: unknown }).error;
      ...
      // Cause chain (APIConnectionError wraps the real network error here)
      const cause = (value as { cause?: unknown }).cause;
      if (cause !== undefined && depth < 4) {
        parts.push(`cause=(${formatForLog(cause, depth + 1)})`);
      }
