...
export function resolveHooksConfig(cfg: WebBotConfig): HooksConfigResolved | null {
...
function resolveKnownAgentIds(cfg: WebBotConfig, defaultAgentId: string): Set<string> {
...
export function extractHookToken(req: IncomingMessage): string | undefined {
  ...
  const headerToken =
    typeof req.headers["x-webbot-token"] === "string"
      ? req.headers["x-webbot-token"].trim()
      : "";
  if (headerToken) {
    return headerToken;
  }
