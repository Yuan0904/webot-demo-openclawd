...
async function withTempConfig(params: { cfg: unknown; run: () => Promise<void> }): Promise<void> {
  const prevConfigPath = process.env.WEBBOT_CONFIG_PATH;
  const prevDisableCache = process.env.WEBBOT_DISABLE_CONFIG_CACHE;

  const dir = await mkdtemp(path.join(os.tmpdir(), "webbot-canvas-auth-test-"));
  const configPath = path.join(dir, "webbot.json");
  ...
  try {
    ...
    if (prevConfigPath === undefined) {
      ...
      process.env.WEBBOT_CONFIG_PATH = prevConfigPath;
    }
    if (prevDisableCache === undefined) {
      delete process.env.WEBBOT_DISABLE_CONFIG_CACHE;
    } else {
      process.env.WEBBOT_DISABLE_CONFIG_CACHE = prevDisableCache;
    }
