...
describe("gateway hot reload", () => {
  ...
  beforeEach(() => {
    prevSkipChannels = process.env.WEBBOT_SKIP_CHANNELS;
    prevSkipGmail = process.env.WEBBOT_SKIP_GMAIL_WATCHER;
    process.env.WEBBOT_SKIP_CHANNELS = "0";
  ...
  afterEach(() => {
    if (prevSkipChannels === undefined) {
      ...
      process.env.WEBBOT_SKIP_CHANNELS = prevSkipChannels;
    }
    if (prevSkipGmail === undefined) {
      delete process.env.WEBBOT_SKIP_GMAIL_WATCHER;
    } else {
      process.env.WEBBOT_SKIP_GMAIL_WATCHER = prevSkipGmail;
    }
  });
