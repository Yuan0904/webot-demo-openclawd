...
describe("onboard (non-interactive): gateway and remote auth", () => {
  ...
  const initStateDir = async (prefix: string) => {
    ...
    process.env.WEBBOT_STATE_DIR = stateDir;
  ...
  beforeAll(async () => {
    ...
    tempHome = await fs.mkdtemp(path.join(os.tmpdir(), "webbot-onboard-"));
  ...
  afterAll(async () => {
    ...
    process.env.WEBBOT_GATEWAY_PASSWORD = prev.password;
  });
  ...
  it("writes gateway token auth into config and gateway enforces it", async () => {
    ...
    const workspace = path.join(stateDir, "webbot");
  ...
  it("auto-generates token auth when binding LAN and persists the token", async () => {
    ...
    process.env.WEBBOT_STATE_DIR = stateDir;
    process.env.WEBBOT_CONFIG_PATH = path.join(stateDir, "webbot.json");
