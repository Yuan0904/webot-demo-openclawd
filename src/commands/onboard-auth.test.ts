...
const requireAgentDir = () => {
  const agentDir = process.env.WEBBOT_AGENT_DIR;
  if (!agentDir) {
    throw new Error("WEBBOT_AGENT_DIR not set");
  }
  return agentDir;
};

describe("writeOAuthCredentials", () => {
  const previousStateDir = process.env.WEBBOT_STATE_DIR;
  const previousAgentDir = process.env.WEBBOT_AGENT_DIR;
  ...
  afterEach(async () => {
    ...
    if (previousStateDir === undefined) {
      ...
      process.env.WEBBOT_STATE_DIR = previousStateDir;
    }
    if (previousAgentDir === undefined) {
      delete process.env.WEBBOT_AGENT_DIR;
    } else {
      process.env.WEBBOT_AGENT_DIR = previousAgentDir;
    }
    ...
    delete process.env.WEBBOT_OAUTH_DIR;
  });

  it("writes auth-profiles.json under WEBBOT_AGENT_DIR when set", async () => {
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "webbot-oauth-"));
    process.env.WEBBOT_STATE_DIR = tempStateDir;
    process.env.WEBBOT_AGENT_DIR = path.join(tempStateDir, "agent");
    process.env.PI_CODING_AGENT_DIR = process.env.WEBBOT_AGENT_DIR;
...
describe("setMinimaxApiKey", () => {
  ...
  it("writes to WEBBOT_AGENT_DIR when set", async () => {
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "webbot-minimax-"));
    process.env.WEBBOT_STATE_DIR = tempStateDir;
    process.env.WEBBOT_AGENT_DIR = path.join(tempStateDir, "custom-agent");
    process.env.PI_CODING_AGENT_DIR = process.env.WEBBOT_AGENT_DIR;
