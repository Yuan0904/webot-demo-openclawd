...
async function collectChannelStatus(params: {
  ...
  const statusEntries = await Promise.all(
    listChannelOnboardingAdapters()
      .filter((adapter) => adapter.getStatus)
      .map((adapter) =>
      adapter.getStatus!({
        cfg: params.cfg,
...
async function noteChannelPrimer(
  ...
  await prompter.note(
    [
      ...
      `Approve with: ${formatCliCommand("webbot pairing approve <channel> <code>")}`,
  ...
  const selectPolicy = async (policy: ChannelOnboardingDmPolicy) => {
    await prompter.note(
      [
        ...
        `Approve: ${formatCliCommand(`webbot pairing approve ${policy.channel} <code>`)}`,
    ...
    const choice = (await prompter.select({
      ...
      options: [
        ...buildSelectionOptions(entries),
        {
          value: "__skip__",
          label: "Skip for now",
          hint: `You can add channels later via \`${formatCliCommand("webbot channels add")}\``,
        },
      ],
