    ...
    await select({
      ...
      options: [
        ...
        },
        {
          ...
          hint: "Delete channel tokens/settings from webbot.json",
        },
      ],
...
async function promptWebToolsConfig(
  ...
  note(
    [
      ...
      "Docs: https://docs.webbot.ai/tools/web",
    ].join("\n"),
    "Web search",
  );
...
export async function runConfigureWizard(
  ...
  try {
    ...
    intro(opts.command === "update" ? "WebBot update wizard" : "WebBot configure");
    const prompter = createClackPrompter();

    const snapshot = await readConfigFileSnapshot();
    const baseConfig: WebBotConfig = snapshot.valid ? snapshot.config : {};
    ...
    if (snapshot.exists) {
      ...
      if (!snapshot.valid && snapshot.issues.length > 0) {
        note(
          [
            ...
            "",
            "Docs: https://docs.webbot.ai/gateway/configuration",
          ].join("\n"),
          "Config issues",
        );
      }
      if (!snapshot.valid) {
        outro(
          `Config invalid. Run \`${formatCliCommand("webbot doctor")}\` to repair it, then re-run configure.`,
        );
        runtime.exit(1);
        return;
      }
    ...
    const localProbe = await probeGatewayReachable({
      url: localUrl,
      token: baseConfig.gateway?.auth?.token ?? process.env.WEBBOT_GATEWAY_TOKEN,
      password: baseConfig.gateway?.auth?.password ?? process.env.WEBBOT_GATEWAY_PASSWORD,
    });
    ...
    let gatewayToken: string | undefined =
      ...
      process.env.WEBBOT_GATEWAY_TOKEN;
    ...
    if (opts.sections) {
      ...
      if (selected.includes("health")) {
        ...
        const token = nextConfig.gateway?.auth?.token ?? process.env.WEBBOT_GATEWAY_TOKEN;
        const password =
          nextConfig.gateway?.auth?.password ?? process.env.WEBBOT_GATEWAY_PASSWORD;
        ...
        try {
          ...
          note(
            [
              "Docs:",
              "https://docs.webbot.ai/gateway/health",
              "https://docs.webbot.ai/gateway/troubleshooting",
            ].join("\n"),
            "Health check help",
          );
        }
    ...
    const newPassword = nextConfig.gateway?.auth?.password ?? process.env.WEBBOT_GATEWAY_PASSWORD;
    const oldPassword = baseConfig.gateway?.auth?.password ?? process.env.WEBBOT_GATEWAY_PASSWORD;
    const token = nextConfig.gateway?.auth?.token ?? process.env.WEBBOT_GATEWAY_TOKEN;
    ...
    note(
      [
        ...
        gatewayStatusLine,
        "Docs: https://docs.webbot.ai/web/control-ui",
      ].join("\n"),
      "Control UI",
    );
