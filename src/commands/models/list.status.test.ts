...
const mocks = vi.hoisted(() => {
  ...
  return {
    store,
    resolveWebBotAgentDir: vi.fn().mockReturnValue("/tmp/webbot-agent"),
    resolveAgentDir: vi.fn().mockReturnValue("/tmp/webbot-agent"),
    ...
    resolveAuthStorePathForDisplay: vi
      .fn()
      .mockReturnValue("/tmp/webbot-agent/auth-profiles.json"),
...
vi.mock("../../agents/agent-paths.js", () => ({
  resolveWebBotAgentDir: mocks.resolveWebBotAgentDir,
}));
...
describe("modelsStatusCommand auth overview", () => {
  it("includes masked auth sources in JSON output", async () => {
    ...
    expect(mocks.resolveWebBotAgentDir).toHaveBeenCalled();
    expect(payload.defaultModel).toBe("anthropic/claude-opus-4-5");
    expect(payload.auth.storePath).toBe("/tmp/webbot-agent/auth-profiles.json");
  ...
  it("uses agent overrides and reports sources", async () => {
    ...
    mocks.resolveAgentDir.mockReturnValue("/tmp/webbot-agent-custom");
    ...
    try {
      ...
      expect(payload.agentDir).toBe("/tmp/webbot-agent-custom");
