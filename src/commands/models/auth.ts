...
import { readConfigFileSnapshot, type WebBotConfig } from "../../config/config.js";
...
function applyDefaultModel(cfg: WebBotConfig, model: string): WebBotConfig {
...
export async function modelsAuthLoginCommand(opts: LoginOptions, runtime: RuntimeEnv) {
  ...
  if (providers.length === 0) {
    throw new Error(
      `No provider plugins found. Install one via \`${formatCliCommand("webbot plugins install")}\`.`,
    );
  }
