...
export async function modelsStatusCommand(
  ...
  const agentDir = agentId ? resolveAgentDir(cfg, agentId) : resolveWebBotAgentDir();
  ...
  if (missingProvidersInUse.length > 0) {
    ...
    for (const provider of missingProvidersInUse) {
      const hint =
        provider === "anthropic"
          ? `Run \`claude setup-token\`, then \`${formatCliCommand("webbot models auth setup-token")}\` or \`${formatCliCommand("webbot configure")}\`.`
          : `Run \`${formatCliCommand("webbot configure")}\` or set an API key env var.`;
