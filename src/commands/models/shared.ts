...
export async function updateConfig(
  mutator: (cfg: WebBotConfig) => WebBotConfig,
...
export function resolveModelTarget(params: { raw: string; cfg: WebBotConfig }): {
  provider: string;
  model: string;
} {
...
export function buildAllowlistSet(cfg: WebBotConfig): Set<string> {
...
export function resolveKnownAgentId(params: {
  ...
  if (!knownAgents.includes(agentId)) {
    throw new Error(
      `Unknown agent id "${raw}". Use "${formatCliCommand("webbot agents list")}" to see configured agents.`,
    );
  }
  return agentId;
}
