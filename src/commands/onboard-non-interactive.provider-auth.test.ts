...
function captureEnv(): EnvSnapshot {
  return {
    ...
    password: process.env.WEBBOT_GATEWAY_PASSWORD,
    customApiKey: process.env.CUSTOM_API_KEY,
    disableConfigCache: process.env.WEBBOT_DISABLE_CONFIG_CACHE,
  };
}
...
function restoreEnv(prev: EnvSnapshot): void {
  ...
  restoreEnvVar("WEBBOT_STATE_DIR", prev.stateDir);
  restoreEnvVar("WEBBOT_CONFIG_PATH", prev.configPath);
  restoreEnvVar("WEBBOT_SKIP_CHANNELS", prev.skipChannels);
  restoreEnvVar("WEBBOT_SKIP_GMAIL_WATCHER", prev.skipGmail);
  restoreEnvVar("WEBBOT_SKIP_CRON", prev.skipCron);
  restoreEnvVar("WEBBOT_SKIP_CANVAS_HOST", prev.skipCanvas);
  restoreEnvVar("WEBBOT_GATEWAY_TOKEN", prev.token);
  restoreEnvVar("WEBBOT_GATEWAY_PASSWORD", prev.password);
  restoreEnvVar("CUSTOM_API_KEY", prev.customApiKey);
  restoreEnvVar("WEBBOT_DISABLE_CONFIG_CACHE", prev.disableConfigCache);
}
...
async function withOnboardEnv(
  ...
  process.env.WEBBOT_DISABLE_CONFIG_CACHE = "1";
  delete process.env.WEBBOT_GATEWAY_TOKEN;
  delete process.env.WEBBOT_GATEWAY_PASSWORD;
  delete process.env.CUSTOM_API_KEY;

  const tempHome = await fs.mkdtemp(path.join(os.tmpdir(), prefix));
  const configPath = path.join(tempHome, "webbot.json");
  process.env.HOME = tempHome;
  process.env.WEBBOT_STATE_DIR = tempHome;
  process.env.WEBBOT_CONFIG_PATH = configPath;
  vi.resetModules();
...
describe("onboard (non-interactive): provider auth", () => {
  it("stores xAI API key and sets default model", async () => {
    await withOnboardEnv("webbot-onboard-xai-", async ({ configPath, runtime }) => {
  ...
  it("stores Vercel AI Gateway API key and sets default model", async () => {
    await withOnboardEnv("webbot-onboard-ai-gateway-", async ({ configPath, runtime }) => {
      await runNonInteractive(
        {
          nonInteractive: true,
  ...
  it("stores token auth profile", async () => {
    await withOnboardEnv("webbot-onboard-token-", async ({ configPath, runtime }) => {
  ...
  it("stores OpenAI API key and sets OpenAI default model", async () => {
    await withOnboardEnv("webbot-onboard-openai-", async ({ configPath, runtime }) => {
      await runNonInteractive(
        {
          nonInteractive: true,
  ...
  it("stores Cloudflare AI Gateway API key and metadata", async () => {
    await withOnboardEnv("webbot-onboard-cf-gateway-", async ({ configPath, runtime }) => {
      await runNonInteractive(
        {
          nonInteractive: true,
  ...
  it("infers Cloudflare auth choice from API key flags", async () => {
    await withOnboardEnv("webbot-onboard-cf-gateway-infer-", async ({ configPath, runtime }) => {
      await runNonInteractive(
        {
          nonInteractive: true,
          cloudflareAiGatewayAccountId: "cf-account-id",
  ...
  it("configures a custom provider from non-interactive flags", async () => {
    await withOnboardEnv("webbot-onboard-custom-provider-", async ({ configPath, runtime }) => {
      await runNonInteractive(
        {
          nonInteractive: true,
  ...
  it("infers custom provider auth choice from custom flags", async () => {
    await withOnboardEnv(
      "webbot-onboard-custom-provider-infer-",
  ...
  it("uses CUSTOM_API_KEY env fallback for non-interactive custom provider auth", async () => {
    await withOnboardEnv(
      "webbot-onboard-custom-provider-env-fallback-",
      async ({ configPath, runtime }) => {
  ...
  it("uses matching profile fallback for non-interactive custom provider auth", async () => {
    await withOnboardEnv(
      "webbot-onboard-custom-provider-profile-fallback-",
      async ({ configPath, runtime }) => {
  ...
  it("fails custom provider auth when compatibility is invalid", async () => {
    await withOnboardEnv(
      "webbot-onboard-custom-provider-invalid-compat-",
      async ({ runtime }) => {
        await expect(
          runNonInteractive(
            {
  ...
  it("fails custom provider auth when explicit provider id is invalid", async () => {
    await withOnboardEnv("webbot-onboard-custom-provider-invalid-id-", async ({ runtime }) => {
      await expect(
        runNonInteractive(
          {
            nonInteractive: true,
  ...
  it("fails inferred custom auth when required flags are incomplete", async () => {
    await withOnboardEnv(
      "webbot-onboard-custom-provider-missing-required-",
      async ({ runtime }) => {
        await expect(
          runNonInteractive(
            {
