...
const SEARCH_TOOL = "https://docs.webbot.ai/mcp.SearchWebBot";
...
export async function docsSearchCommand(queryParts: string[], runtime: RuntimeEnv) {
  ...
  if (!query) {
    const docs = formatDocsLink("/", "docs.webbot.ai");
    if (isRich()) {
      runtime.log(`${theme.muted("Docs:")} ${docs}`);
      runtime.log(`${theme.muted("Search:")} ${formatCliCommand('webbot docs "your query"')}`);
    } else {
      runtime.log("Docs: https://docs.webbot.ai/");
      runtime.log(`Search: ${formatCliCommand('webbot docs "your query"')}`);
    }
    return;
  }
