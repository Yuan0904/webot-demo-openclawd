...
function resolveMode(cfg: WebBotConfig): "local" | "remote" {
...
export async function doctorCommand(
  ...
  const root = await resolveWebBotPackageRoot({
  ...
  let cfg: WebBotConfig = configResult.cfg;

  const configPath = configResult.path ?? CONFIG_PATH;
  if (!cfg.gateway?.mode) {
    const lines = [
      "gateway.mode is unset; gateway start will be blocked.",
      `Fix: run ${formatCliCommand("webbot configure")} and set Gateway mode (local/remote).`,
      `Or set directly: ${formatCliCommand("webbot config set gateway.mode local")}`,
    ];
    if (!fs.existsSync(configPath)) {
      lines.push(`Missing config: run ${formatCliCommand("webbot setup")} first.`);
    }
    note(lines.join("\n"), "Gateway");
  }
  ...
  if (shouldWriteConfig) {
    ...
    runtime.log(`Run "${formatCliCommand("webbot doctor --fix")}" to apply changes.`);
  }
