...
import type { WebBotConfig, GatewayBindMode } from "../config/config.js";
...
export async function noteSecurityWarnings(cfg: WebBotConfig) {
  const warnings: string[] = [];
  const auditHint = `- Run: ${formatCliCommand("webbot security audit --deep")}`;
    ...
    if (!hasSharedSecret) {
      const authFixLines =
        resolvedAuth.mode === "password"
          ? [
              `  Fix: ${formatCliCommand("webbot configure")} to set a password`,
              `  Or switch to token: ${formatCliCommand("webbot config set gateway.auth.mode token")}`,
            ]
          : [
              `  Fix: ${formatCliCommand("webbot doctor --fix")} to generate a token`,
              `  Or set token directly: ${formatCliCommand(
                "webbot config set gateway.auth.mode token",
              )}`,
            ];
      warnings.push(
        `- CRITICAL: Gateway bound to ${bindDescriptor} without authentication.`,
        `  Anyone on your network (or internet if port-forwarded) can fully control your agent.`,
        `  Fix: ${formatCliCommand("webbot config set gateway.bind loopback")}`,
        ...authFixLines,
      );
    } else {
