...
beforeEach(() => {
  ...
  resolveWebBotPackageRoot.mockReset().mockResolvedValue(null);
  ...
  setStdinTty(true);
  originalStateDir = process.env.WEBBOT_STATE_DIR;
  originalUpdateInProgress = process.env.WEBBOT_UPDATE_IN_PROGRESS;
  process.env.WEBBOT_UPDATE_IN_PROGRESS = "1";
  tempStateDir = fs.mkdtempSync(path.join(os.tmpdir(), "webbot-doctor-state-"));
...
afterEach(() => {
  ...
  if (originalStateDir === undefined) {
    ...
    process.env.WEBBOT_STATE_DIR = originalStateDir;
  }
  if (originalUpdateInProgress === undefined) {
    delete process.env.WEBBOT_UPDATE_IN_PROGRESS;
  } else {
    process.env.WEBBOT_UPDATE_IN_PROGRESS = originalUpdateInProgress;
  }
  if (tempStateDir) {
...
const resolveWebBotPackageRoot = vi.fn().mockResolvedValue(null);
...
vi.mock("../plugins/loader.js", () => ({
  loadWebBotPlugins: () => ({ plugins: [], diagnostics: [] }),
}));
...
vi.mock("../infra/webbot-root.js", () => ({
  resolveWebBotPackageRoot,
}));
...
describe("doctor command", () => {
  it("warns when the state directory is missing", async () => {
    ...
    const missingDir = fs.mkdtempSync(path.join(os.tmpdir(), "webbot-missing-state-"));
    fs.rmSync(missingDir, { recursive: true, force: true });
    process.env.WEBBOT_STATE_DIR = missingDir;
    note.mockClear();
  ...
  it("skips gateway auth warning when WEBBOT_GATEWAY_TOKEN is set", async () => {
    readConfigFileSnapshot.mockResolvedValue({
      path: "/tmp/webbot.json",
      exists: true,
      raw: "{}",
    ...
    process.env.WEBBOT_GATEWAY_TOKEN = "env-token-1234567890";
    note.mockClear();
