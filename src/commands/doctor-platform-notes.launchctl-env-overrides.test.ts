...
describe("noteMacLaunchctlGatewayEnvOverrides", () => {
  it("prints clear unsetenv instructions for token override", async () => {
    ...
    const getenv = vi.fn(async (name: string) =>
      name === "WEBBOT_GATEWAY_TOKEN" ? "launchctl-token" : undefined,
    );
    const cfg = {
      gateway: {
        auth: {
    ...
    expect(message).toContain("WEBBOT_GATEWAY_TOKEN");
    expect(message).toContain("launchctl unsetenv WEBBOT_GATEWAY_TOKEN");
    expect(message).not.toContain("WEBBOT_GATEWAY_PASSWORD");
  });
