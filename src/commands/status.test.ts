...
beforeAll(() => {
  previousProfile = process.env.WEBBOT_PROFILE;
  process.env.WEBBOT_PROFILE = "isolated";
});
...
afterAll(() => {
  if (previousProfile === undefined) {
    ...
    process.env.WEBBOT_PROFILE = previousProfile;
  }
});
...
vi.mock("../infra/webbot-root.js", () => ({
  resolveWebBotPackageRoot: vi.fn().mockResolvedValue("/tmp/webbot"),
}));
...
vi.mock("../infra/update-check.js", () => ({
  checkUpdateStatus: vi.fn().mockResolvedValue({
    ...
    deps: {
      manager: "pnpm",
      status: "ok",
      lockfilePath: "/tmp/webbot/pnpm-lock.yaml",
      markerPath: "/tmp/webbot/node_modules/.modules.yaml",
    },
...
describe("statusCommand", () => {
  ...
  it("prints formatted lines otherwise", async () => {
    ...
    expect(logs.some((l) => l.includes("WebBot status"))).toBe(true);
    ...
    expect(
      logs.some(
        (l) =>
          l.includes("webbot status --all") ||
          l.includes("webbot --profile isolated status --all") ||
          l.includes("webbot status --all") ||
          l.includes("webbot --profile isolated status --all"),
      ),
    ).toBe(true);
  ...
  it("shows gateway auth when reachable", async () => {
    ...
    process.env.WEBBOT_GATEWAY_TOKEN = "abcd1234";
    try {
