...
export function listAgentEntries(cfg: WebBotConfig): AgentEntry[] {
...
function resolveAgentName(cfg: WebBotConfig, agentId: string) {
...
function resolveAgentModel(cfg: WebBotConfig, agentId: string) {
  const entry = listAgentEntries(cfg).find(
    (agent) => normalizeAgentId(agent.id) === normalizeAgentId(agentId),
  );
  if (entry?.model) {
...
export function buildAgentSummaries(cfg: WebBotConfig): AgentSummary[] {
