...
export async function statusCommand(
  ...
  runtime.log(theme.heading("WebBot status"));
  runtime.log("");
  ...
  runtime.log(theme.muted(`Full report: ${formatCliCommand("webbot security audit")}`));
  runtime.log(theme.muted(`Deep probe: ${formatCliCommand("webbot security audit --deep")}`));
  ...
  runtime.log("");
  runtime.log("FAQ: https://docs.webbot.ai/faq");
  runtime.log("Troubleshooting: https://docs.webbot.ai/troubleshooting");
  runtime.log("");
  ...
  runtime.log(`  Need to share?      ${formatCliCommand("webbot status --all")}`);
  runtime.log(`  Need to debug live? ${formatCliCommand("webbot logs --follow")}`);
  if (gatewayReachable) {
    runtime.log(`  Need to test channels? ${formatCliCommand("webbot status --deep")}`);
  } else {
    runtime.log(`  Fix reachability first: ${formatCliCommand("webbot gateway probe")}`);
  }
}
