import type { BrowserConfig, BrowserProfileConfig, WebBotConfig } from "../config/config.js";
...
import {
  ...
  DEFAULT_WEBBOT_BROWSER_ENABLED,
...
export type ResolvedBrowserProfile = {
  ...
  cdpIsLoopback: boolean;
  color: string;
  driver: "webbot" | "extension";
};

function normalizeHexColor(raw: string | undefined) {
  const value = (raw ?? "").trim();
  if (!value) {
    return DEFAULT_WEBBOT_BROWSER_COLOR;
  }
...
/**
 * Ensure the default "webbot" profile exists in the profiles map.
...
function ensureDefaultProfile(
  ...
  if (!result[DEFAULT_WEBBOT_BROWSER_PROFILE_NAME]) {
    result[DEFAULT_WEBBOT_BROWSER_PROFILE_NAME] = {
 ...
 *
 * Note: this is an WebBot browser profile (routing config), not a Chrome user profile.
...
function ensureDefaultChromeExtensionProfile(
  ...
  // (legacy single-profile configs may use controlPort+1 for webbot/webbot CDP).
...
export function resolveBrowserConfig(
  ...
  rootConfig?: WebBotConfig,
): ResolvedBrowserConfig {
  const enabled = cfg?.enabled ?? DEFAULT_WEBBOT_BROWSER_ENABLED;
...
export function resolveProfile(
  ...
  let cdpUrl = "";
  const driver = profile.driver === "extension" ? "extension" : "webbot";
