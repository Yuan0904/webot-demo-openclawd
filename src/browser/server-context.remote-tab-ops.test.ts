...
vi.mock("./chrome.js", () => ({
  ...
  launchWebBotChrome: vi.fn(async () => {
  ...
  return {
    ...
    resolved: {
      ...
      profiles: {
        ...
        webbot: { cdpPort: 18800, color: "#FF4500" },
      },
    },
    profiles: new Map(),
  };
...
describe("browser server-context tab selection state", () => {
  it("updates lastTargetId when openTab is created via CDP", async () => {
    ...
    const { createBrowserRouteContext } = await import("./server-context.js");
    const state = makeState("webbot");
    const ctx = createBrowserRouteContext({ getState: () => state });
    const webbot = ctx.forProfile("webbot");

    const opened = await webbot.openTab("https://created.example");
    expect(opened.targetId).toBe("CREATED");
    expect(state.profiles.get("webbot")?.lastTargetId).toBe("CREATED");
  });
});
