...
function decoratedMarkerPath(userDataDir: string) {
  return path.join(userDataDir, ".webbot-profile-decorated");
}
 ...
 */
export function decorateWebBotProfile(
  userDataDir: string,
  opts?: { name?: string; color?: string },
) {
  const desiredName = opts?.name ?? DEFAULT_WEBBOT_BROWSER_PROFILE_NAME;
  const desiredColor = (opts?.color ?? DEFAULT_WEBBOT_BROWSER_COLOR).toUpperCase();
