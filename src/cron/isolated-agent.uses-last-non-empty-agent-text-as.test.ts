...
async function withTempHome<T>(fn: (home: string) => Promise<T>): Promise<T> {
  return withTempHomeBase(fn, { prefix: "webbot-cron-" });
}

async function writeSessionStore(home: string) {
  const dir = path.join(home, ".webbot", "sessions");
...
function makeCfg(
  home: string,
  storePath: string,
  overrides: Partial<WebBotConfig> = {},
): WebBotConfig {
  const base: WebBotConfig = {
    agents: {
      defaults: {
        model: "anthropic/claude-opus-4-5",
        workspace: path.join(home, "webbot"),
      },
    },
...
describe("runCronIsolatedAgentTurn", () => {
  ...
  it("uses agentId for workspace, session key, and store paths", async () => {
    await withTempHome(async (home) => {
      ...
      const cfg = makeCfg(
        home,
        path.join(home, ".webbot", "agents", "{agentId}", "sessions", "sessions.json"),
        {
          agents: {
