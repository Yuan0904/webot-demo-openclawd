...
export function buildServiceEnvironment(params: {
  ...
  const stateDir = env.WEBBOT_STATE_DIR;
  const configPath = env.WEBBOT_CONFIG_PATH;
  return {
    HOME: env.HOME,
    PATH: buildMinimalServicePath({ env }),
    WEBBOT_PROFILE: profile,
    WEBBOT_STATE_DIR: stateDir,
    WEBBOT_CONFIG_PATH: configPath,
    WEBBOT_GATEWAY_PORT: String(port),
    WEBBOT_GATEWAY_TOKEN: token,
    WEBBOT_LAUNCHD_LABEL: resolvedLaunchdLabel,
    WEBBOT_SYSTEMD_UNIT: systemdUnit,
    WEBBOT_SERVICE_MARKER: GATEWAY_SERVICE_MARKER,
    WEBBOT_SERVICE_KIND: GATEWAY_SERVICE_KIND,
    WEBBOT_SERVICE_VERSION: VERSION,
  };
}
...
export function buildNodeServiceEnvironment(params: {
  ...
  return {
    ...
    WEBBOT_STATE_DIR: stateDir,
    WEBBOT_CONFIG_PATH: configPath,
    WEBBOT_LAUNCHD_LABEL: resolveNodeLaunchAgentLabel(),
    WEBBOT_SYSTEMD_UNIT: resolveNodeSystemdServiceName(),
    WEBBOT_WINDOWS_TASK_NAME: resolveNodeWindowsTaskName(),
    WEBBOT_TASK_SCRIPT_NAME: NODE_WINDOWS_TASK_SCRIPT_NAME,
    WEBBOT_LOG_PREFIX: "node",
    WEBBOT_SERVICE_MARKER: NODE_SERVICE_MARKER,
    WEBBOT_SERVICE_KIND: NODE_SERVICE_KIND,
    WEBBOT_SERVICE_VERSION: VERSION,
  };
}
