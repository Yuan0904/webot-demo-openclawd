...
describe("buildServiceEnvironment", () => {
  it("sets minimal PATH and gateway vars", () => {
    ...
    expect(env.WEBBOT_GATEWAY_PORT).toBe("18789");
    expect(env.WEBBOT_GATEWAY_TOKEN).toBe("secret");
    expect(env.WEBBOT_SERVICE_MARKER).toBe("webbot");
    expect(env.WEBBOT_SERVICE_KIND).toBe("gateway");
    expect(typeof env.WEBBOT_SERVICE_VERSION).toBe("string");
    expect(env.WEBBOT_SYSTEMD_UNIT).toBe("webbot-gateway.service");
    if (process.platform === "darwin") {
      expect(env.WEBBOT_LAUNCHD_LABEL).toBe("ai.webbot.gateway");
    }
  });

  it("uses profile-specific unit and label", () => {
    const env = buildServiceEnvironment({
      env: { HOME: "/home/user", WEBBOT_PROFILE: "work" },
      port: 18789,
    });
    expect(env.WEBBOT_SYSTEMD_UNIT).toBe("webbot-gateway-work.service");
    if (process.platform === "darwin") {
      expect(env.WEBBOT_LAUNCHD_LABEL).toBe("ai.webbot.work");
    }
  });
});
