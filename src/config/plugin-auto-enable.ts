import type { WebBotConfig } from "./config.js";
...
function isTelegramConfigured(cfg: WebBotConfig, env: NodeJS.ProcessEnv): boolean {
...
function isDiscordConfigured(cfg: WebBotConfig, env: NodeJS.ProcessEnv): boolean {
...
function isIrcConfigured(cfg: WebBotConfig, env: NodeJS.ProcessEnv): boolean {
...
function isSlackConfigured(cfg: WebBotConfig, env: NodeJS.ProcessEnv): boolean {
  if (
...
function isSignalConfigured(cfg: WebBotConfig): boolean {
...
function isIMessageConfigured(cfg: WebBotConfig): boolean {
...
function isWhatsAppConfigured(cfg: WebBotConfig): boolean {
...
function isGenericChannelConfigured(cfg: WebBotConfig, channelId: string): boolean {
...
function collectModelRefs(cfg: WebBotConfig): string[] {
  const refs: string[] = [];
...
function isProviderConfigured(cfg: WebBotConfig, providerId: string): boolean {
...
function isPluginExplicitlyDisabled(cfg: WebBotConfig, pluginId: string): boolean {
  const entry = cfg.plugins?.entries?.[pluginId];
  return entry?.enabled === false;
}

function isPluginDenied(cfg: WebBotConfig, pluginId: string): boolean {
...
function ensureAllowlisted(cfg: WebBotConfig, pluginId: string): WebBotConfig {
...
function registerPluginEntry(cfg: WebBotConfig, pluginId: string): WebBotConfig {
  const entries = {
    ...cfg.plugins?.entries,
    [pluginId]: {
