...
describe("config env vars", () => {
  ...
  it("loads ${VAR} substitutions from ~/.webbot/.env on repeated runtime loads", async () => {
    await withTempHome(async (home) => {
      await withEnvOverride(
        {
          WEBBOT_STATE_DIR: path.join(home, ".webbot"),
          CLAWDBOT_STATE_DIR: undefined,
          WEBBOT_HOME: undefined,
          CLAWDBOT_HOME: undefined,
          BRAVE_API_KEY: undefined,
          WEBBOT_DISABLE_CONFIG_CACHE: "1",
        },
        async () => {
