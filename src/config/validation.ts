...
import type { WebBotConfig, ConfigValidationIssue } from "./types.js";
...
import { WebBotSchema } from "./zod-schema.js";
...
function validateIdentityAvatar(config: WebBotConfig): ConfigValidationIssue[] {
...
export function validateConfigObject(
  raw: unknown,
): { ok: true; config: WebBotConfig } | { ok: false; issues: ConfigValidationIssue[] } {
  ...
  const validated = WebBotSchema.safeParse(raw);
  ...
  const duplicates = findDuplicateAgentDirs(validated.data as WebBotConfig);
  ...
  const avatarIssues = validateIdentityAvatar(validated.data as WebBotConfig);
  ...
  return {
    ...
    config: applyModelDefaults(
      applyAgentDefaults(applySessionDefaults(validated.data as WebBotConfig)),
    ),
  };
}
