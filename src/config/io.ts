...
import type { WebBotConfig, ConfigFileSnapshot, LegacyConfigIssue } from "./types.js";
...
import { compareWebBotVersions } from "./version.js";
...
function coerceConfig(value: unknown): WebBotConfig {
  if (!value || typeof value !== "object" || Array.isArray(value)) {
    return {};
  }
  return value as WebBotConfig;
}
...
function stampConfigVersion(cfg: WebBotConfig): WebBotConfig {
...
function warnIfConfigFromFuture(cfg: WebBotConfig, logger: Pick<typeof console, "warn">): void {
  const touched = cfg.meta?.lastTouchedVersion;
  if (!touched) {
    return;
  }
  const cmp = compareWebBotVersions(VERSION, touched);
  if (cmp === null) {
    return;
  }
  if (cmp < 0) {
    logger.warn(
      `Config was last written by a newer WebBot (${touched}); current version is ${VERSION}.`,
    );
  }
}

function applyConfigEnv(cfg: WebBotConfig, env: NodeJS.ProcessEnv): void {
...
export function createConfigIO(overrides: ConfigIoDeps = {}) {
  ...
  function loadConfig(): WebBotConfig {
    try {
      ...
      if (resolved && typeof resolved === "object" && "env" in resolved) {
        applyConfigEnv(resolved as WebBotConfig, deps.env);
      }
      ...
      const preValidationDuplicates = findDuplicateAgentDirs(resolvedConfig as WebBotConfig, {
        env: deps.env,
  ...
  async function writeConfigFile(cfg: WebBotConfig) {
    clearConfigCache();
...
// module scope. `WEBBOT_CONFIG_PATH` (and friends) are expected to work even
...
function resolveConfigCacheMs(env: NodeJS.ProcessEnv): number {
  const raw = env.WEBBOT_CONFIG_CACHE_MS?.trim();
  if (raw === "" || raw === "0") {
    return 0;
  }
  if (!raw) {
...
function shouldUseConfigCache(env: NodeJS.ProcessEnv): boolean {
  if (env.WEBBOT_DISABLE_CONFIG_CACHE?.trim()) {
    return false;
  }
...
export function loadConfig(): WebBotConfig {
...
export async function writeConfigFile(cfg: WebBotConfig): Promise<void> {
  clearConfigCache();
