...
// Mock loadConfig so resolveMaintenanceConfig() never reads a real webbot.json.
...
describe("rotateSessionFile", () => {
  ...
  beforeEach(async () => {
    testDir = await fs.mkdtemp(path.join(os.tmpdir(), "webbot-rotate-"));
...
describe("Integration: saveSessionStore with pruning", () => {
  ...
  beforeEach(async () => {
    testDir = await fs.mkdtemp(path.join(os.tmpdir(), "webbot-pruning-integ-"));
    storePath = path.join(testDir, "sessions.json");
    savedCacheTtl = process.env.WEBBOT_SESSION_CACHE_TTL_MS;
    process.env.WEBBOT_SESSION_CACHE_TTL_MS = "0";
    clearSessionStoreCacheForTest();
  ...
  afterEach(async () => {
    ...
    if (savedCacheTtl === undefined) {
      delete process.env.WEBBOT_SESSION_CACHE_TTL_MS;
    } else {
      process.env.WEBBOT_SESSION_CACHE_TTL_MS = savedCacheTtl;
    }
  });
