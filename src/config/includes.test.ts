...
const ETC_WEBBOT_DIR = path.join(ROOT_DIR, "etc", "webbot");
const SHARED_DIR = path.join(ROOT_DIR, "shared");

const DEFAULT_BASE_PATH = path.join(CONFIG_DIR, "webbot.json");

function configPath(...parts: string[]) {
  return path.join(CONFIG_DIR, ...parts);
}

function etcWebBotPath(...parts: string[]) {
  return path.join(ETC_WEBBOT_DIR, ...parts);
}
...
describe("resolveConfigIncludes", () => {
  ...
  it("resolves absolute path $include", () => {
    const absolute = etcWebBotPath("agents.json");
  ...
  it("resolves parent directory references", () => {
    ...
    expect(resolve(obj, files, configPath("sub", "webbot.json"))).toEqual({
      shared: true,
    });
  });
});
