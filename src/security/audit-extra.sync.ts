...
function summarizeGroupPolicy(cfg: WebBotConfig): {
  open: number;
  allowlist: number;
  other: number;
} {
...
function collectModels(cfg: WebBotConfig): ModelRef[] {
  const out: ModelRef[] = [];
...
function hasWebSearchKey(cfg: WebBotConfig, env: NodeJS.ProcessEnv): boolean {
...
function isWebSearchEnabled(cfg: WebBotConfig, env: NodeJS.ProcessEnv): boolean {
...
function isWebFetchEnabled(cfg: WebBotConfig): boolean {
...
function isBrowserEnabled(cfg: WebBotConfig): boolean {
  try {
...
function listGroupPolicyOpen(cfg: WebBotConfig): string[] {
  const out: string[] = [];
  const channels = cfg.channels as Record<string, unknown> | undefined;
  if (!channels || typeof channels !== "object") {
    return out;
...
export function collectAttackSurfaceSummaryFindings(cfg: WebBotConfig): SecurityAuditFinding[] {
...
export function collectSyncedFolderFindings(params: {
  ...
  if (isProbablySyncedPath(params.stateDir) || isProbablySyncedPath(params.configPath)) {
    findings.push({
      ...
      remediation: `Keep WEBBOT_STATE_DIR on a local-only volume and re-run "${formatCliCommand("webbot security audit --fix")}".`,
    });
  }
  return findings;
}

export function collectSecretsInConfigFindings(cfg: WebBotConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
  const password =
  ...
  if (password && !looksLikeEnvRef(password)) {
    findings.push({
      ...
      remediation:
        "Prefer WEBBOT_GATEWAY_PASSWORD (env) and remove gateway.auth.password from disk.",
    });
  }
...
export function collectHooksHardeningFindings(cfg: WebBotConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
...
export function collectModelHygieneFindings(cfg: WebBotConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
...
export function collectExposureMatrixFindings(cfg: WebBotConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
