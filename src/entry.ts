...
function ensureExperimentalWarningSuppressed(): boolean {
  if (isTruthyEnvValue(process.env.WEBBOT_NO_RESPAWN)) {
    return false;
  }
  if (isTruthyEnvValue(process.env.WEBBOT_NODE_OPTIONS_READY)) {
    return false;
  }
  ...
  process.env.WEBBOT_NODE_OPTIONS_READY = "1";
...
if (!ensureExperimentalWarningSuppressed()) {
  ...
  if (!parsed.ok) {
    ...
    console.error(`[webbot] ${parsed.error}`);
    process.exit(2);
  }
