...
export async function runGatewayUpdate(opts: UpdateRunnerOptions = {}): Promise<UpdateRunResult> {
  ...
  if (gitRoot && pkgRoot && path.resolve(gitRoot) === path.resolve(pkgRoot)) {
    ...
    if (channel === "dev") {
      ...
      const preflightRoot = await fs.mkdtemp(path.join(os.tmpdir(), "webbot-update-preflight-"));
    ...
    const doctorEntry = path.join(gitRoot, "webbot.mjs");
    ...
    const doctorStep = await runStep(
      step("webbot doctor", doctorArgv, gitRoot, { WEBBOT_UPDATE_IN_PROGRESS: "1" }),
    );
