...
export function triggerWebBotRestart(): RestartAttempt {
  ...
  if (process.platform !== "darwin") {
    if (process.platform === "linux") {
      const unit = normalizeSystemdUnit(
        process.env.WEBBOT_SYSTEMD_UNIT,
        process.env.WEBBOT_PROFILE,
      );
  ...
  const label =
    process.env.WEBBOT_LAUNCHD_LABEL ||
    resolveGatewayLaunchAgentLabel(process.env.WEBBOT_PROFILE);
