...
export type HeartbeatRunner = {
  stop: () => void;
  updateConfig: (cfg: WebBotConfig) => void;
};

function hasExplicitHeartbeatAgents(cfg: WebBotConfig) {
  const list = cfg.agents?.list ?? [];
  return list.some((entry) => Boolean(entry?.heartbeat));
}

export function isHeartbeatEnabledForAgent(cfg: WebBotConfig, agentId?: string): boolean {
...
function resolveHeartbeatAgents(cfg: WebBotConfig): HeartbeatAgent[] {
  const list = cfg.agents?.list ?? [];
...
export function resolveHeartbeatPrompt(cfg: WebBotConfig, heartbeat?: HeartbeatConfig) {
  return resolveHeartbeatPromptText(heartbeat?.prompt ?? cfg.agents?.defaults?.heartbeat?.prompt);
}

function resolveHeartbeatAckMaxChars(cfg: WebBotConfig, heartbeat?: HeartbeatConfig) {
  return Math.max(
    0,
...
export function startHeartbeatRunner(opts: {
  ...
  const updateConfig = (cfg: WebBotConfig) => {
    if (state.stopped) {
      return;
    }
    const now = Date.now();
