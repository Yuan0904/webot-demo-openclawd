...
async function withTempDir<T>(run: (dir: string) => Promise<T>): Promise<T> {
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "webbot-run-node-"));
  try {
  ...
  it.runIf(process.platform !== "win32")(
    ...
    async () => {
      await withTempDir(async (tmp) => {
        ...
        await fs.writeFile(
          path.join(tmp, "webbot.mjs"),
          "#!/usr/bin/env node\nif (process.argv.includes('--version')) console.log('9.9.9-test');\n",
          "utf-8",
        );
        await fs.chmod(path.join(tmp, "webbot.mjs"), 0o755);
