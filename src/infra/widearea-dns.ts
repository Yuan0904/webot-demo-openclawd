...
export function resolveWideAreaDiscoveryDomain(params?: {
  ...
  const candidate = params?.configDomain ?? env.WEBBOT_WIDE_AREA_DOMAIN ?? null;
...
function extractContentHash(zoneText: string): string | null {
  const match = zoneText.match(/^\s*;\s*webbot-content-hash:\s*(\S+)\s*$/m);
  return match?.[1] ?? null;
}
...
function renderZone(opts: WideAreaGatewayZoneOpts & { serial: number }): string {
  const hostname = os.hostname().split(".")[0] ?? "webbot";
  const hostLabel = dnsLabel(opts.hostLabel ?? hostname, "webbot");
  const instanceLabel = dnsLabel(opts.instanceLabel ?? `${hostname}-gateway`, "webbot-gw");
  ...
  records.push(`_webbot-gw._tcp IN PTR ${instanceLabel}._webbot-gw._tcp`);
  records.push(`${instanceLabel}._webbot-gw._tcp IN SRV 0 0 ${opts.gatewayPort} ${hostLabel}`);
  records.push(`${instanceLabel}._webbot-gw._tcp IN TXT ${txt.map(txtQuote).join(" ")}`);
  ...
  return `; webbot-content-hash: ${contentHash}\n${contentBody}`;
}
