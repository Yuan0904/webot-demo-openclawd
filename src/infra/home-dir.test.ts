...
describe("resolveEffectiveHomeDir", () => {
  it("prefers WEBBOT_HOME over HOME and USERPROFILE", () => {
    const env = {
  ...
  it("expands WEBBOT_HOME when set to ~", () => {
    const env = {
      WEBBOT_HOME: "~/svc",
      HOME: "/home/alice",
    } as NodeJS.ProcessEnv;
...
describe("resolveRequiredHomeDir", () => {
  ...
  it("returns a fully resolved path for WEBBOT_HOME", () => {
    const result = resolveRequiredHomeDir(
      { WEBBOT_HOME: "/custom/home" } as NodeJS.ProcessEnv,
      () => "/fallback",
    );
    expect(result).toBe(path.resolve("/custom/home"));
  });

  it("returns cwd when WEBBOT_HOME is tilde-only and no fallback home exists", () => {
    expect(
      resolveRequiredHomeDir({ WEBBOT_HOME: "~" } as NodeJS.ProcessEnv, () => {
        throw new Error("no home");
      }),
    ).toBe(process.cwd());
  });
...
describe("expandHomePrefix", () => {
  it("expands tilde using effective home", () => {
    const value = expandHomePrefix("~/x", {
      env: { WEBBOT_HOME: "/srv/webbot-home" } as NodeJS.ProcessEnv,
    });
    expect(value).toBe(`${path.resolve("/srv/webbot-home")}/x`);
  });
