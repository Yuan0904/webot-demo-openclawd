...
function candidateBinDirs(opts: EnsureWebBotPathOpts): string[] {
  ...
  // Bundled macOS app: `webbot` lives next to the executable (process.execPath).
  try {
    const execDir = path.dirname(execPath);
    const siblingCli = path.join(execDir, "webbot");
  ...
  // Project-local installs (best effort): if a `node_modules/.bin/webbot` exists near cwd,
  // include it. This helps when running under launchd or other minimal PATH environments.
  const localBinDir = path.join(cwd, "node_modules", ".bin");
  if (isExecutable(path.join(localBinDir, "webbot"))) {
...
/**
 * Best-effort PATH bootstrap so skills that require the `webbot` CLI can run
 * under launchd/minimal environments (and inside the macOS app bundle).
 */
export function ensureWebBotCliOnPath(opts: EnsureWebBotPathOpts = {}) {
  if (isTruthyEnvValue(process.env.WEBBOT_PATH_BOOTSTRAPPED)) {
    return;
  }
  process.env.WEBBOT_PATH_BOOTSTRAPPED = "1";
