...
#
# Suggested cron: */30 * * * * /home/admin/webbot/scripts/auth-monitor.sh
#
# Environment variables:
#   NOTIFY_PHONE - Phone number to send WebBot notification (e.g., +1234567890)
#   NOTIFY_NTFY  - ntfy.sh topic for push notifications (e.g., webbot-alerts)
...
STATE_FILE="$HOME/.webbot/auth-monitor-state"
...
send_notification() {
    ...
    # Send via WebBot if phone configured and auth still valid
    if [ -n "$NOTIFY_PHONE" ]; then
        # Check if we can still use webbot
        if "$SCRIPT_DIR/claude-auth-status.sh" simple 2>/dev/null | grep -q "OK\|EXPIRING"; then
            echo "Sending via WebBot to $NOTIFY_PHONE..."
            webbot send --to "$NOTIFY_PHONE" --message "$message" 2>/dev/null || true
        fi
    fi
...
if [ "$DIFF_MS" -lt 0 ]; then
    send_notification "Claude Code auth EXPIRED! WebBot is down. Run: ssh l36 '~/webbot/scripts/mobile-reauth.sh'" "urgent"
    exit 1
