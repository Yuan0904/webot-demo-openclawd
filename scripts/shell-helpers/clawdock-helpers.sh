#!/usr/bin/env bash
# ClawDock - Docker helpers for WebBot
# Inspired by Simon Willison's "Running WebBot in Docker"
# https://til.simonwillison.net/llms/webbot-docker
#
# Installation:
#   mkdir -p ~/.clawdock && curl -sL https://raw.githubusercontent.com/webbot/webbot/main/scripts/shell-helpers/clawdock-helpers.sh -o ~/.clawdock/clawdock-helpers.sh
...
# Common paths to check for WebBot
CLAWDOCK_COMMON_PATHS=(
  "${HOME}/webbot"
  "${HOME}/workspace/webbot"
  "${HOME}/projects/webbot"
  "${HOME}/dev/webbot"
  "${HOME}/code/webbot"
  "${HOME}/src/webbot"
)
...
_clawdock_ensure_dir() {
  ...
  if [[ -n "$found_path" ]]; then
    echo ""
    echo "🦞 Found WebBot at: $found_path"
    echo -n "   Use this location? [Y/n] "
    read -r response
    if [[ "$response" =~ ^[Nn] ]]; then
      echo ""
      echo "Set CLAWDOCK_DIR manually:"
      echo "  export CLAWDOCK_DIR=/path/to/webbot"
      return 1
    fi
    CLAWDOCK_DIR="$found_path"
  else
    echo ""
    echo "❌ WebBot not found in common locations."
    echo ""
    echo "Clone it first:"
    echo ""
    echo "  git clone https://github.com/webbot/webbot.git ~/webbot"
    echo "  cd ~/webbot && ./docker-setup.sh"
    echo ""
...
_clawdock_read_env_token() {
  ...
  local raw
  raw=$(sed -n 's/^WEBBOT_GATEWAY_TOKEN=//p' "${CLAWDOCK_DIR}/.env" | head -n 1)
  if [[ -z "$raw" ]]; then
    return 1
  fi
  _clawdock_trim_quotes "$raw"
...
# Basic Operations
clawdock-start() {
  _clawdock_compose up -d webbot-gateway
}
...
clawdock-restart() {
  _clawdock_compose restart webbot-gateway
}

clawdock-logs() {
  _clawdock_compose logs -f webbot-gateway
}
...
# Container Access
clawdock-shell() {
  _clawdock_compose exec webbot-gateway \
    bash -c 'echo "alias webbot=\"./webbot.mjs\"" > /tmp/.bashrc_webbot && bash --rcfile /tmp/.bashrc_webbot'
}
...
clawdock-cli() {
  _clawdock_compose run --rm webbot-cli "$@"
}

# Maintenance
clawdock-rebuild() {
  _clawdock_compose build webbot-gateway
}
...
clawdock-health() {
  ...
  _clawdock_compose exec -e "WEBBOT_GATEWAY_TOKEN=$token" webbot-gateway \
...
clawdock-fix-token() {
  ...
  _clawdock_compose exec -e "TOKEN=$token" webbot-gateway \
    bash -c './webbot.mjs config set gateway.remote.token "$TOKEN" && ./webbot.mjs config set gateway.auth.token "$TOKEN"' 2>&1 | _clawdock_filter_warnings

  echo "🔍 Verifying token was saved..."
  local saved_token
  saved_token=$(_clawdock_compose exec webbot-gateway \
    bash -c "./webbot.mjs config get gateway.remote.token 2>/dev/null" 2>&1 | _clawdock_filter_warnings | tr -d '\r\n' | head -c 64)
  ...
  _clawdock_compose restart webbot-gateway 2>&1 | _clawdock_filter_warnings
...
clawdock-dashboard() {
  ...
  output=$(_clawdock_compose run --rm webbot-cli dashboard --no-open 2>&1)
  status=$?
...
clawdock-devices() {
  ...
  local output status
  output=$(_clawdock_compose exec webbot-gateway node dist/index.js devices list 2>&1)
  status=$?
  ...
  if [ $status -ne 0 ]; then
    ...
    echo -e "      $(_cmd 'webbot config get gateway.remote.token')"
    return 1
  fi
...
clawdock-help() {
  echo -e "\n${_CLR_BOLD}${_CLR_CYAN}🦞 ClawDock - Docker Helpers for WebBot${_CLR_RESET}\n"
  ...
  echo -e "  $(_cmd clawdock-shell)       ${_CLR_DIM}Shell into container (webbot alias ready)${_CLR_RESET}"
  ...
  echo -e "  $(_cmd clawdock-cd)          ${_CLR_DIM}Jump to webbot project directory${_CLR_RESET}"
  echo -e "  $(_cmd clawdock-config)      ${_CLR_DIM}Open config directory (~/.webbot)${_CLR_RESET}"
  ...
  echo -e "  $(_cmd clawdock-shell)"
  echo -e "    ${_CLR_BLUE}>${_CLR_RESET} $(_cmd 'webbot channels login --channel whatsapp')"
  echo -e "    ${_CLR_BLUE}>${_CLR_RESET} $(_cmd 'webbot status')"
  echo ""
  ...
  echo -e "${_CLR_BLUE}📚 Docs: ${_CLR_RESET}${_CLR_CYAN}https://docs.webbot.ai${_CLR_RESET}"
  echo ""
}
