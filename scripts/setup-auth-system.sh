#!/bin/bash
# Setup WebBot Auth Management System
...
echo "  2. Subscribe to a topic (e.g., 'webbot-alerts')"
echo ""
...
# Phone notification
echo ""
echo "  WebBot message: Send warning via WebBot itself"
echo "Enter your phone number for alerts (or leave blank to skip):"
read -r PHONE_NUMBER

# Update service file
SERVICE_FILE="$SCRIPT_DIR/systemd/webbot-auth-monitor.service"
...
cp "$SCRIPT_DIR/systemd/webbot-auth-monitor.service" ~/.config/systemd/user/
cp "$SCRIPT_DIR/systemd/webbot-auth-monitor.timer" ~/.config/systemd/user/
systemctl --user daemon-reload
systemctl --user enable --now webbot-auth-monitor.timer
...
echo "  - Auth monitor: systemctl --user status webbot-auth-monitor.timer"
echo ""
echo "Quick commands:"
